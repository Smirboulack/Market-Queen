...
///
/// One sentence, handed to the avatar model as its motion prompt. It used to
/// sit beside a personality -- traits, an energy slider, a line of speaking
/// style -- and that whole block is gone. It read as though it shaped the
/// performance and it did not: it reached the script writer and the image
/// model, never the voice and never the movement, so an actor set to high
/// energy was written about energetically and then read exactly like
/// everybody else. Rather than wire four more couplings to make the promise
/// true, the promise is withdrawn. Delivery is directed where it can be
/// heard: in the script, with tags, on the engine that reads them.
class ActorAction {
  ActorAction._();

  static const key = 'actorAction';

  static String of(LibraryAsset actor) => actor.extraText(key).trim();
}
